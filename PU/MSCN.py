...
for i in range(1, 11):
    ...
    input_layer = Input(shape=(15, 15, 15, 1), name='Pavia_Input')
...
#第一层
    ...
    conv_layer13 = Conv3D(filters=16, kernel_size=(3, 3, 15))(relu12)
...
#全连接层
    ...
    output_layer = Dense(9, activation='softmax')(dropout2)
...
#反向传播
    ...
    filepath = 'TKSN-15x15x15-10%' + "Window Size" + str(windowSize) + "num" + str(numPCAcomponents)\
               + str(i) + '.hdf5'
    #reduce_lr = ReduceLROnPlateau(monitor=*'val_acc', factor=0.5, patience=2, min_lr=0.0001, verbose=1)
    checkpoint = ModelCheckpoint(filepath, monitor='accuracy', verbose=1, save_best_only=True, mode='max')
    callbacks_list = [checkpoint]
    start = time.time()
    history = model.fit(x=X_train, y=y_train, verbose=1, batch_size=16, epochs=40,
