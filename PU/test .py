...
for i in range(1, 11):
    ...
    model = load_model('TKSN-15x15x15-10%' + "Window Size" + str(windowSize) + "num" + str(numPCAcomponents)\
               + str(i) + '.hdf5')

    classification, confusion, Test_loss, Test_accuracy, oa, each_acc, aa, kappa = reports(X_test, y_test)
    classification = str(classification)
    confusion = str(confusion)
    file_name ='TKSN-15x15x15-10%' + "Window Size" + str(windowSize) + "num" + str(numPCAcomponents)\
               + str(i) + ".txt"
...
#
 #   spectral.save_rgb("TKRN-PU.jpg", outputs.astype(int), colors=spectral.spy_colors)
