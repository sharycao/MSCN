...
for i in range(1, 11):
    ...
    #model = load_model('TKSN-25x25x25' + "Window Size" + str(windowSize) + "num" + str(numPCAcomponents) + str(i) + '.hdf5')
    model = load_model('TKSN-25x25x25-0.70' + "Window Size" + str(windowSize) + "num" + str(numPCAcomponents)\
               + str(i)  + '.hdf5')

    classification, confusion, Test_loss, Test_accuracy, oa, each_acc, aa, kappa = reports(X_test, y_test)
    classification = str(classification)
    confusion = str(confusion)
    file_name = 'TKSN-25x25x25-0.70' + "Window Size" + str(windowSize) + "PCA" +str(numPCAcomponents) + str(i) + '.txt'
...
'''
    ...
    spectral.save_rgb("TKRN.jpg", outputs.astype(int), colors=spectral.spy_colors)
